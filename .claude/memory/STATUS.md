...
| 22 | Section 4 rebuild — "Vet Recommended This Month" becomes "Our Veterinarians' Top Picks" with the practice-voice subtitle and an "Explore the Pet Store" CTA. Recommendation badge rotates Most Recommended / Trusted Choice. **BE:** new `utils/reviewStats.js` attaches real `ratingAvg` + `reviewCount` to `GET /products` via one aggregate per page (products had no rating field at all); 0/0 when unreviewed so the client hides the row. **FE:** `ProductCardV2` gains opt-in props (`recommendBadge`, `ratingAvg`/`reviewCount`, `categories`, `quantity`/`lowStockThreshold`) rendering pet-type chips, a stars row and In Stock / Selling Fast / Out of Stock — other callers pass none of them and are unchanged. "Select options" → "View Details" globally. "Shop by Need" filter matches by category first, keyword-in-name as fallback, and hides needs that match nothing (Joint Care is hidden today). New shared `CategoryTabs` sliding-pill switcher now used by BOTH this section and FeaturedProductSection (the duplicate `fp-tab*` implementation is gone). **Data reality:** only 5 reviews exist DB-wide and 4 of their products were deleted, so no live product shows a rating yet; life stages (Kitten/Adult/Senior) are stored in the `colors` field, so puppy/senior icons were deliberately not built. Not built: "Ask the VitalPaws Assistant" (depends on the NOT NOW Section 3 assistant). Source doc: `Vet Recommended (4).docx`. | BE + FE ✅ |
| Fix | Mega menu clipped at the right edge — CSS `translateX(-50%)` centring was silently overwritten by framer-motion's inline transform (so no mega menu was ever centred), and the edge-clamp measured a mid-animation `scale(0.98)` box and under-corrected. Centring + clamping now both computed as marginLeft from `offsetWidth`. | FE ✅ |

| 23 | Veterinarian Network — homepage section retitled "Find a Veterinarian You Can Trust" and filtered to role=veterinarian (old list was top-by-rating across ALL roles — mostly groomers/trainers). Card: stars + reviewCount, clinicName, locality (address tail), Available Today + real hours from `availability`, species chips; Book Appointment = tel: link to the vet's own number (wa.me fallback), View Profile → existing detail page; avatar rail replaced with Previous/Next + N/M counter. **BE:** `professionalInfo.clinicName` + `speciesTreated` (optional, admin-validated, flow through getProfessionalData spread). Admin form gains both inputs. Profile page enriched (rating, clinic, address, species, Call/WhatsApp/Directions) and fixes `pro.phone` → `phoneNumber` (phone line never rendered). Full image1 profile design (Clinic entity, galleries, vet reviews, articles, FAQs) deliberately deferred. Source doc: `Veterinarian Network (6).docx`. | BE + FE ✅ |
| Fix | Account dropdown double-panel — MegaMenu (floating panel) was nested inside .nav-user-menu (another floating panel): detached white box, nested role=menu, double animation. MegaMenu gains an `inline` variant (list only, shared columnsBlock). Currency dropdown `<ul>` lacked a list reset — default bullets showed. | FE ✅ |

| 24 | Pet Care Tips → "Veterinarian Advice" — new heading + subtitle ("Trusted veterinary advice, practical guides and health tips for happier pets"); the 01/02/03 counter replaced by a coloured category badge, one colour per value of the `petCareTip` category enum (nutrition/grooming/health/training/exercise/dental/behavior) plus a neutral fallback; class lowercased so stray capitals still colour. Category removed from the meta line (was duplicated by the badge) — now "dog · 4 min read". Badge kept on phones (the old counter was hidden there) with its fixed width dropped. FE only — every tip already has a required enum category. | FE ✅ |
